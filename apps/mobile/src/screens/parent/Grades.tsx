...
export const GradesScreen: React.FC = () => {
  ...
  const { grades, isLoading, loadGrades } = useStudentStore();
  ...
  // Load grades when child ID changes
  useEffect(() => {
    if (childStudentId) {
  ...
  const subjectsData = useMemo(() => {
    ...
    subjectMap.forEach(subject => {
      const validScores = subject.grades
        .map(g => g.score !== null ? (g.score / g.maxScore) * 10 : 0)
        .filter(s => s > 0);
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <View className="bg-gradient-to-br from-[#0284C7] to-[#0369A1] pt-[60px] px-6 pb-6 rounded-b-[30px]">
        ...
        <Text className="text-[12px] text-blue-100 font-medium mt-0.5">
          {selectedChild ? `${selectedChild.name} • ${selectedChild.grade}${selectedChild.section}` : 'Năm học 2025 - 2026'}
        </Text>
      </View>
